...
def es_espurio(c):
    ...
    if c in (' ', chr(92), chr(36), chr(39), chr(96), chr(34), '~', '-'):
        return True
...
def parsear_linea_item(linea):
    ...
    # Código: solo si es alfanumérico puro (sin espacios ni símbolos)
    codigo = antes_cant if re.match(r'^[A-Za-z0-9]{2,20}$', antes_cant) else None
