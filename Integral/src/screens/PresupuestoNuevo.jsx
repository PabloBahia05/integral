...
function EspecialesMedidasForm({
  tipo, icono, titulo,
  medidas, setMedidas,
  materialSearch, setMaterialSearch, materialFocus, setMaterialFocus, materialesFiltrados,
  guiasSearch, setGuiasSearch, guiasFocus, setGuiasFocus, guiasFiltradas,
  onVolver, onContinuar,
}) {
  const inputStyle = {
    width: "100%",
  ...
  const fieldWrap = { flex: "1 1 120px", display: "flex", flexDirection: "column" };

  const canContinue =
    medidas.ancho && medidas.alto && medidas.profundidad && medidas.materialPlaca;
  ...
  return (
    <div style={{ fontFamily: "'Space Mono',monospace" }}>
      ...
      <div style={{
        ...
        background: "#e8f0f7", border: "1px solid #c8dae8", borderRadius: 3,
        padding: "12px 18px", marginBottom: 20,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 22 }}>{icono}</span>
          <span style={{ fontWeight: 700, fontSize: 14, color: "#0a3a5c", textTransform: "uppercase", letterSpacing: "0.06em" }}>
            {titulo}
          </span>
          <span style={{ fontSize: 11, color: "#6699bb", marginLeft: 4 }}>— Medidas y materiales</span>
        </div>
        <button
          onClick={onVolver}
          style={{
            padding: "5px 14px", background: "#fff", border: "1px solid #b8cfe0",
            borderRadius: 2, fontFamily: "'Space Mono',monospace", fontSize: 12,
            cursor: "pointer", color: "#0a3a5c",
          }}
        >← Volver</button>
      </div>
      ...
      {/* Formulario */}
      <div style={{
        background: "#f5f9fc", border: "1px solid #c8dae8", borderRadius: 4, padding: "22px 24px",
      }}>
        {/* Fila: medidas numéricas */}
        <div style={{ marginBottom: 6, fontSize: 10, color: "#8aabcc", textTransform: "uppercase", letterSpacing: "0.1em" }}>
          Medidas (cm)
        </div>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 0 }}>
          {/* Material Placa */}
          <div style={{ ...fieldWrap, flex: "2 1 220px", position: "relative" }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      {codigo && nombre !== codigo && (
                        <span style={{ color: "#8aabcc", marginLeft: 8, fontSize: 10 }}>{codigo}</span>
                      )}
                    </div>
                  );
                })}
            ...
            {materialFocus && materialesFiltrados.length === 0 && materialSearch.length > 0 && (
              <div style={{ ...dropdownStyle, padding: "10px 14px", color: "#8aabcc", fontSize: 11 }}>
          ...
          <div style={{ ...fieldWrap, flex: "2 1 220px", position: "relative" }}>
            ...
            {guiasFocus && guiasFiltradas.length > 0 && (
              <div style={dropdownStyle}>
                {guiasFiltradas.map((g, i) => {
                  const nombre = g.nombreart ?? g.NOMBREART ?? g.articulo ?? g.ARTICULO ?? "";
            ...
            {guiasFocus && guiasFiltradas.length === 0 && guiasSearch.length > 0 && (
              <div style={{ ...dropdownStyle, padding: "10px 14px", color: "#8aabcc", fontSize: 11 }}>
                Sin resultados — se usará el texto ingresado
              </div>
            )}
      ...
      {(medidas.ancho || medidas.alto || medidas.profundidad || medidas.materialPlaca || medidas.guias) && (
        <div style={{
          marginTop: 16, background: "#fff", border: "1px solid #c8dae8",
          borderRadius: 3, padding: "12px 18px",
          display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap",
        }}>
          <span style={{ fontSize: 10, color: "#6699bb", textTransform: "uppercase", letterSpacing: "0.08em" }}>Resumen:</span>
          {medidas.ancho && <span style={{ fontSize: 12, color: "#0a3a5c" }}>📐 <b>{medidas.ancho}</b> × <b>{medidas.alto || "—"}</b> × <b>{medidas.profundidad || "—"}</b> cm</span>}
          {medidas.materialPlaca && <span style={{ fontSize: 12, color: "#0a3a5c" }}>🪵 <b>{medidas.materialPlaca}</b></span>}
          {medidas.guias && <span style={{ fontSize: 12, color: "#0a3a5c" }}>🔩 <b>{medidas.guias}</b></span>}
        </div>
      )}
      ...
      <div style={{ marginTop: 20, display: "flex", justifyContent: "flex-end", gap: 10 }}>
        <button
          onClick={onVolver}
          style={{
            padding: "8px 20px", background: "#fff", border: "1px solid #b8cfe0",
            borderRadius: 2, fontFamily: "'Space Mono',monospace", fontSize: 12,
            cursor: "pointer", color: "#0a3a5c",
          }}
        >Cancelar</button>
      ...
      {!canContinue && (
        <div style={{ marginTop: 8, textAlign: "right", fontSize: 10, color: "#c0392b", fontFamily: "'Space Mono',monospace" }}>
...
export default function PresupuestoNuevo({
  ...
  const materialesFiltrados = materialesDB.filter(m =>
  ...
  const guiasFiltradas = guiasDB.filter(g =>
          ...
          {tab === "especiales" && especialesVista === "selector" && (
            ...
            >
              ...
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    if (key === "vanitory") {
                      ...
                      // Para escritorio/despensero, inicializar medidas y navegar directo
                ...
                >
                  {key === "wallpanel"
                    ? <img src={WALLPANEL_IMG} alt="Wall Panel" style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 4 }} />
